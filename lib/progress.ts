...
export type JourneyPracticeProgress = {
...
export function readProgress(): UserProgress {
  ...
  try {
    ...
    return parsed.version === 2 ? parsed : emptyProgress;
  } catch {
...
export function recordConceptVisit(conceptId: string): UserProgress {
  ...
  return writeProgress({
    ...current,
    visitedConceptIds: Array.from(
      new Set([...current.visitedConceptIds, conceptId]),
    ),
    lastLocation: '#explore',
  });
}
...
export function recordFoundationVisit(foundationId: string): UserProgress {
  ...
  return writeProgress({
    ...current,
    foundationVisits: Array.from(
      new Set([...current.foundationVisits, foundationId]),
    ),
  });
}
...
export function recordJourneyStop(
  ...
  const completedStopIds =
    completed && existing
      ? Array.from(new Set([...existing.completedStopIds, stopId]))
      : (existing?.completedStopIds ?? []);
  return writeProgress({
    ...current,
    journeyProgress: {
      ...current.journeyProgress,
      [journeyId]: { currentStopId: stopId, completedStopIds },
    },
    lastLocation: `#/journey/${journeyId}/${stopId}`,
  });
}

export function readJourneyPractice(
  journeyId: string,
  stopId: string,
): JourneyPracticeProgress {
  return (
    readProgress().journeyPractice?.[`${journeyId}/${stopId}`] ?? {
      workedExample: false,
      guidedComplete: false,
      independentComplete: false,
    }
  );
}
...
export function recordJourneyPractice(
  ...
  const existing = current.journeyPractice?.[key] ?? {
    workedExample: false,
    guidedComplete: false,
    independentComplete: false,
  };
  return writeProgress({
    ...current,
    journeyPractice: {
      ...current.journeyPractice,
      [key]: { ...existing, [step]: true },
    },
  });
}
