...
const stopDiagramNode: Record<string, string> = {
  'question-orientation': 'sources',
  'choose-store': 'stores',
  'clean-and-shape': 'stores',
  'build-star-schema': 'stores',
  'model-relationships': 'model',
  'add-a-measure': 'model',
  'trace-the-query': 'model',
  'control-access': 'model',
  'diagnose-performance': 'model',
};
...
export function HomePage() {
  ...
  const activeDiagramNode = currentStop
    ? stopDiagramNode[currentStop.id]
    : undefined;
      ...
      {navOpen ? (
        <div className="nav-drawer" aria-label="Primary navigation">
          ...
          <nav aria-label="Primary navigation">
            <a className="active" href="#/" onClick={() => setNavOpen(false)}>
              Home
            </a>
            <a href="#/journeys" onClick={() => setNavOpen(false)}>
              <Compass aria-hidden="true" />
              Field Trips
            </a>
            <a href="#/foundations" onClick={() => setNavOpen(false)}>
              Foundations
            </a>
            <a href="#/map" onClick={() => setNavOpen(false)}>
              Map
            </a>
      ...
      <main className="home-main">
        <section className="home-hero">
          <div className="home-cards">
            <article className="home-card home-card-primary">
              <p className="eyebrow">
                <Compass aria-hidden="true" />
                {started ? 'Continue the field trip' : 'Start the field trip'}
              </p>
              <h1>{journey.title}</h1>
              <p>
                {started && currentStop
                  ? `You're on stop "${currentStop.title}." ${journey.missionSummary}`
                  : journey.missionSummary}
              </p>
              <div className="home-card-actions">
                <Button
                  size="lg"
                  onClick={() => {
                    window.location.hash = `#/journey/${journey.id}/${resumeStopId}`;
                  }}
                >
                  {started ? 'Continue journey' : 'Start journey'}
                  <ArrowRight />
                </Button>
                <a className="home-card-secondary-link" href="#/journeys">
                  What will I learn?
                </a>
              </div>
            </article>
            <article className="home-card home-card-secondary">
              ...
              <p>
                The full interactive Fabric Atlas and lab list stay available
                any time you want to explore off-script.
              </p>
              <a className="home-card-link" href="#explore">
                Explore freely
                <ArrowRight />
              </a>
            </article>
          ...
          >
            {diagramNodes.map((node, index) => (
              <div key={node.id} className="home-diagram-item">
                <span
                  className={`home-diagram-node ${activeDiagramNode === node.id ? 'is-current' : ''}`}
                >
                  {node.label}
        ...
        {!foundationSeen ? (
          <aside className="home-foundation-nudge">
            <p>
              <strong>
                New to semantic models, star schemas, or Fabric terms?
              </strong>{' '}
              Spend 8–12 minutes on the foundation bridge first.
            </p>
            <a href="#/foundations">
              Open Foundation Bridge
              <ArrowRight />
            </a>
          </aside>
        ) : null}
