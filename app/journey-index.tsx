    ...
    <div className="lab-page journey-index-page">
      <header className="lab-topbar">
        ...
        <div>
          ...
          <span>Guided Aurora Outfitters journeys</span>
        </div>
      ...
      <main className="journey-index">
        {Object.values(journeys).map((journey) => {
          ...
          const resumeStopId =
            progress?.currentStopId ?? firstStopId(journey.id);
            ...
            <article key={journey.id} className="journey-card">
              ...
              <ol className="journey-stop-list">
                {journey.stopIds.map((stopId, index) => {
                  ...
                  return (
                    <li key={stopId} className={isDone ? 'is-done' : ''}>
                      <span>{index + 1}</span>
                      {stop.title}
                    </li>
                  );
              ...
              <div className="journey-card-actions">
                ...
                >
                  {started ? 'Continue field trip' : 'Start field trip'}
                  <ArrowRight />
                </Button>
                {started ? (
                  <Button
                    variant="ghost"
                    onClick={() => {
                      window.location.hash = `#/journey/${journey.id}/${firstStopId(journey.id)}`;
                    }}
                  >
                    <RotateCcw />
                    Start over
              ...
              <p className="journey-progress-note">
