...
export function reduceStorageMode(
  state: StorageModeState,
  action: StorageModeAction,
): StorageModeState {
  switch (action.type) {
    case 'set-mode':
      return { ...state, mode: action.mode, runCount: 0 };
    case 'set-query':
      return { ...state, query: action.query, runCount: 0 };
    case 'change-source':
      return { ...state, sourceVersion: state.sourceVersion + 1, runCount: 0 };
    case 'refresh-cache':
      return state.mode === 'import' || state.mode === 'dual'
        ? { ...state, cacheVersion: state.sourceVersion, runCount: 0 }
        : state;
    case 'run-query':
      return { ...state, runCount: state.runCount + 1 };
    case 'reset':
      return initialStorageModeState;
  }
}

export type PathStep = {
  id: 'report' | 'model' | 'memory' | 'source';
  eyebrow: string;
  title: string;
  detail: string;
};
...
export function describeStorageMode(state: StorageModeState) {
      ...
      steps: [
        {
          id: 'report',
          eyebrow: 'Request',
          title: isDimensionOnly ? 'Product slicer' : 'Revenue by category',
          detail: 'A report visual issues a DAX query.',
        },
        {
          id: 'model',
          eyebrow: 'Semantic model',
          title: 'Import tables',
          detail: 'Relationships and measures shape the request.',
        },
        {
          id: 'memory',
          eyebrow: 'VertiPaq',
          title: `Imported data · v${state.cacheVersion}`,
          detail: 'Compressed model data answers the query.',
        },
      ] as PathStep[],
    };

  if (state.mode === 'direct-query')
    return {
      ...
      steps: [
        ...
        },
        {
          id: 'model',
          eyebrow: 'Semantic model',
          title: 'DirectQuery metadata',
          detail: 'The model translates the request.',
        },
        {
          id: 'source',
          eyebrow: 'Source',
          title: `Warehouse tables · v${state.sourceVersion}`,
          detail: 'A native query runs at report time.',
        },
      ] as PathStep[],
    };
  ...
  if (state.mode === 'dual') {
    ...
    return {
      title: useCache
        ? 'Dual behaves like Import for this slicer'
        : 'Dual joins the DirectQuery path for this visual',
      ...
      steps: [
        {
          ...
          eyebrow: 'Request',
          title: isDimensionOnly ? 'Product slicer' : 'Revenue by category',
          detail: 'Query context determines Dual behavior.',
        },
        {
          id: 'model',
          eyebrow: 'Composite model',
          title: 'DimProduct · Dual',
          detail: isDimensionOnly
            ? 'No DirectQuery fact is required.'
            : 'FactSales · DirectQuery is required.',
        },
        useCache
          ? {
              id: 'memory',
              eyebrow: 'VertiPaq',
              title: `Dimension cache · v${state.cacheVersion}`,
              detail: 'The cached dimension serves the request.',
            }
          : {
              id: 'source',
              eyebrow: 'Source',
              title: `DimProduct + FactSales · v${state.sourceVersion}`,
              detail: 'One native query combines both tables.',
            },
      ] as PathStep[],
    };
  }
  ...
  return {
    ...
    steps: [
      ...
      },
      {
        id: 'model',
        eyebrow: 'Semantic model',
        title: 'Direct Lake tables',
        detail: 'The model identifies required columns.',
      },
      {
        id: 'memory',
        eyebrow: 'VertiPaq',
        title: 'On-demand column cache',
        detail: 'Needed columns are loaded for fast scans.',
      },
      {
        id: 'source',
        eyebrow: 'OneLake',
        title: `Delta tables · v${state.sourceVersion}`,
        detail: 'Data remains in OneLake rather than a separate import copy.',
      },
    ] as PathStep[],
  };
}

export const storageModeAccuracyNote =
  'Conceptual path model. Actual performance, caching, query folding, and DirectQuery behavior depend on the source, model, capacity, and query.';
