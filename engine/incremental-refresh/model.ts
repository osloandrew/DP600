...
const shiftDays = (value: Date, days: number) =>
  new Date(value.getTime() + days * DAY);

export function evaluateIncrementalRefresh(
  ...
  const archiveStart = new Date(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - policy.storeMonths, 1),
  );
  const historySplit = new Date(
    Date.UTC(refreshStart.getUTCFullYear(), refreshStart.getUTCMonth(), 1),
  );
  ...
  const visibleRevenue =
    lateChange && refreshRun && lateChangeCovered ? 1250 : 1000;
  const processedKinds: PartitionKind[] = refreshRun ? ['refresh'] : [];
  const partitions: IncrementalPartition[] = [
    {
      id: 'archive',
      label: 'Historical archive',
      range: `${iso(archiveStart)} → ${iso(historySplit)}`,
      kind: 'historical',
      processed: false,
    },
  ];
  if (historySplit < refreshStart)
    partitions.push({
      id: 'recent-history',
      label: 'Recent history',
      range: `${iso(historySplit)} → ${iso(refreshStart)}`,
      kind: 'historical',
      processed: false,
    });
  partitions.push({
    ...
    range: `${iso(refreshStart)} → ${iso(now)}`,
    kind: 'refresh',
    processed: refreshRun,
  });
  if (policy.realtime)
    partitions.push({
      id: 'realtime',
...
export const incrementalAccuracyNote =
  'The service creates and manages finer-grained partitions automatically. This compact timeline groups them to make the rolling-window policy observable.';
