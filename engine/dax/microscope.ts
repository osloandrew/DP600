...
export type SalesRow = {
  id: number;
  category: 'Bikes' | 'Helmets' | 'Jerseys';
  quantity: number;
  unitPrice: number;
  revenue: number;
};
...
export const daxScenarios = {
  sum: {
    label: 'SUM',
    expression: 'Total Sales = SUM(FactSales[Revenue])',
    steps: [
      'Start with all visible rows',
      'Read the Revenue column',
      'Aggregate visible values',
    ],
  },
  calculate: {
    label: 'CALCULATE',
    expression:
      'Bike Sales = CALCULATE([Total Sales], DimProduct[Category] = "Bikes")',
    steps: [
      'Existing filter context',
      'Add Category = Bikes',
      'Propagate to FactSales',
  ...
  sumx: {
    label: 'SUMX',
    expression:
      'Extended Sales = SUMX(FactSales, FactSales[Quantity] * FactSales[Unit Price])',
    steps: [
      'Create row context',
      'Evaluate row 10482',
      'Evaluate row 10491',
  ...
  all: {
    label: 'ALL',
    expression:
      'All Category Sales = CALCULATE([Total Sales], ALL(DimProduct[Category]))',
    steps: [
      'Existing filter: Bikes',
      'ALL removes Category filter',
      'All fact rows become visible',
      'Evaluate [Total Sales]',
    ],
  },
} as const;
...
export function evaluateDax(id: DaxScenarioId, step: number) {
  ...
  const visibleRows =
    filterApplied || (id === 'all' && !allRemoved)
      ? salesRows.filter((row) => row.category === 'Bikes')
      : salesRows;
  const currentRow =
    id === 'sumx' && bounded > 0 ? salesRows[bounded - 1] : undefined;
  const accumulator =
    id === 'sumx'
      ? salesRows
          .slice(0, bounded)
  ...
  return {
    ...
    currentValue: currentRow
      ...
      : undefined,
    accumulator,
    resultReady: bounded === scenario.steps.length - 1,
    filterChips:
      id === 'calculate' && filterApplied
        ? ['Category = Bikes']
        : id === 'all' && !allRemoved
          ? ['Category = Bikes']
          : [],
