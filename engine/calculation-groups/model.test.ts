...
describe('calculation group lab', () => {
  it('shows the original combinatorial measure count', () =>
    expect(repetitiveMeasureNames).toHaveLength(12));
  it('applies YTD to the selected base measure', () =>
    expect(evaluateCalculationGroup('margin', 'ytd').value).toBe(2610000));
  ...
  it('preserves the selected measure format for non-percentage items', () => {
    expect(evaluateCalculationGroup('orders', 'current').formatString).toBe(
      '#,0',
    );
