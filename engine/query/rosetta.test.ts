...
describe('Query Rosetta registry', () => {
  it('provides every task for every surface', () => {
    for (const surface of Object.keys(querySurfaceMeta) as QuerySurface[]) {
      for (const task of ['filter', 'aggregate', 'group'] as QueryTask[])
        expect(getQueryScenario(surface, task).code.length).toBeGreaterThan(10);
    }
  });
  ...
  it('keeps language purpose explicit', () => {
    ...
    expect(getQueryScenario('dax', 'group').purpose).toContain(
      'filter context',
    );
    expect(getQueryScenario('visual', 'group').language).toBe(
      'Generates T-SQL',
    );
  });
