...
export function FoundationInline({ ids }: { ids: FoundationConceptId[] }) {
  ...
  const [revealedId, setRevealedId] = useState<FoundationConceptId | null>(
    null,
  );
        ...
        {ids.map((id) => {
          ...
          return (
            <button
              key={id}
              type="button"
              className={`foundation-chip ${isOpen ? 'is-open' : ''}`}
      ...
      {open ? (
        <section className="foundation-panel" aria-label={open.title}>
          ...
          <p>
            ...
            {open.whyItMattersLater}
          </p>
          <div className="foundation-check">
            <p>{open.check.prompt}</p>
            {revealedId === open.id ? (
              <p className="foundation-reveal">{open.check.reveal}</p>
            ) : (
              <button type="button" onClick={() => setRevealedId(open.id)}>
                Reveal answer
              </button>
            )}
          </div>
          <a href={`#/foundations/${open.id}`}>
            Open in Foundation Bridge
            <ArrowRight />
          </a>
        </section>
      ) : null}
