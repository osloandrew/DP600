...
export const examScenarios: ExamScenario[] = [
  ...
  {
    id: 'direct-lake-endpoint-security-fallback', domain: 'Implement and manage semantic models', objectiveIds: ['MODEL.OPTIMIZE.DIRECT_LAKE'], title: 'A Direct Lake report starts routing through the SQL endpoint',
    context: 'Aurora’s Direct Lake model over FactSales normally serves report queries straight from OneLake columns. A security engineer then defines row-level security directly on the SQL analytics endpoint for FactSales. The same report queries now come back through the SQL analytics endpoint instead.',
    question: 'What is the most direct explanation for the shift?',
    options: [option('fallback', 'The model falls back to the SQL analytics endpoint because row-level security is defined there', 'Yes. Row-level security defined at the SQL analytics endpoint is a documented Direct Lake fallback trigger to DirectQuery through that endpoint.'), option('broken', 'Direct Lake is broken and needs to be rebuilt', 'This is documented fallback behavior, not a failure — nothing about the model needs rebuilding.'), option('reimport', 'The model silently switched to a full Import refresh', 'Direct Lake does not convert itself into an Import model; the documented behavior is a query-path fallback, not a refresh-mode change.')],
    answerId: 'fallback', decisiveClue: 'Row-level security applied at the SQL analytics endpoint is the specific, documented condition that routes Direct Lake queries through DirectQuery instead.', remediation: { label: 'Revisit Direct Lake Engine Room', href: '#/lab/direct-lake' },
  },
  {
